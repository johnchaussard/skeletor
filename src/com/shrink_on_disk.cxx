...
int32_t main(int argc, char* argv[])
{
	...
	nrs=(uint64_t)ceil((double)rs/factor); ncs=(uint64_t)ceil((double)cs/factor); nd=(uint64_t)ceil((double)d/factor);
	...
	while(1) //we will break out of the loop
	{
		...
		{
			fprintf(stderr, "Error in crop_on_disk (%ld %ld %ld).\n", i_d, j_d, k_d);
			exit(1);
		}


		i_a_arrivee = (uint64_t)(((((i_d+sid-1)/factor_rs)-1) < (nrs - 1)) ? (((i_d+sid-1)/factor_rs)-1) : (nrs - 1));
		j_a_arrivee = (uint64_t)(((((j_d+sid-1)/factor_cs)-1) < (ncs - 1)) ? (((j_d+sid-1)/factor_cs)-1) : (ncs - 1));
		k_a_arrivee = (uint64_t)(((((k_d+sid-1)/factor_d)-1) < (nd - 1)) ? (((k_d+sid-1)/factor_d)-1) : (nd - 1));

		for(k_a=(uint64_t)ceil(((double)k_d/factor_d)); k_a<=k_a_arrivee; k_a++)
			for(j_a=(uint64_t)ceil(((double)j_d/factor_cs)); j_a<=j_a_arrivee; j_a++)
				for(i_a=(uint64_t)ceil(((double)i_d/factor_rs)); i_a<=i_a_arrivee; i_a++)
				{
					v=0.0;
					somme_facteur=0.0;

					for(k=(uint64_t)(k_a*factor_d); k<=ceil((double)(k_a+1)*factor_d)-1; k++)
					{
						...
						for(j=(uint64_t)(j_a*factor_cs); j<=ceil((double)(j_a+1)*factor_cs)-1; j++)
						{
							...
							for(i=(uint64_t)(i_a*factor_rs); i<=ceil((double)(i_a+1)*factor_rs)-1; i++)
							{
		...
		{
			i_d= (uint64_t)((i_a_arrivee +1)*factor_rs);
		}
		else
		{
			i_d=0;
			...
			{
				j_d= (uint64_t)((j_a_arrivee +1)*factor_cs);
			}
			else
			{
				j_d=0;
				if(k_a_arrivee < (nd -1))
				{
					k_d= (uint64_t)((k_a_arrivee +1)*factor_d);
				}
				else
					break;
			}
