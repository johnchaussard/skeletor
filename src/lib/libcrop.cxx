...
uint32_t read_size_on_disk(FILE *fd, uint64_t *rs, uint64_t *cs, uint64_t *d)
{
	...
	if (fgets(buffer, 3, fd) == NULL) /* P7: raw byte 3d  ; P4: ascii 3d  ==  extensions MC */
	{
		fprintf(stderr, "read_size_on_disk():Error reading image file (header)");
		return(-1);
	}
	...
	if (fgets(buffer, 2, fd) == NULL)
	{
		fprintf(stderr, "read_size_on_disk(): Error reading image file (comments)");
		return(-1);
	}

	if( fgets(buffer, 2, fd) == NULL)
	{
		fprintf(stderr, "read_size_on_disk():Error reading image file (comments 2)");
		return(-1);
	}
	while(buffer[0] == '#')
	{
		while(buffer[0] != '\n')
			if (fgets(buffer, 2, fd) == NULL)
			{
				fprintf(stderr, "read_size_on_disk(): Error reading image file (comments 3)");
				return(-1);
			}

		if (fgets(buffer, 2, fd) == NULL)
		{
			fprintf(stderr, "read_size_on_disk(): Error reading image file (comments 4)");
			return(-1);
		}
    }
	if (fgets(&buffer[1], 100, fd) == NULL)
	{
		fprintf(stderr, "read_size_on_disk(): Error reading image file (size)");
		return(-1);
	}
...
uint32_t crop_on_disk(FILE *fd, struct xvimage** image_decoupee, uint64_t cx, uint64_t cy, uint64_t cz, uint64_t width, uint64_t height, uint64_t dpth)
{
	...
	/* Lecture des 2 premiers octets P7 ou P5*/
	if(fgets(buffer, 3*sizeof(char), fd) == NULL) /* P7: raw byte 3d  ; P4: ascii 3d  ==  extensions MC */
	{
	...
	if (fgets(buffer, 2, fd) == NULL)
	{
		fprintf(stderr, "Error reading image file (comments)");
		free(buffer);
		return(-1);
	}
	...
	if(fgets(buffer, 2, fd) == NULL)
	{
		fprintf(stderr, "Error reading image file (comments 2)");
		free(buffer);
		return(-1);
	}
	...
	while(buffer[0] == '#')
	{
		while(buffer[0] != '\n')
			if (fgets(buffer, 2, fd) == NULL)
			{
				fprintf(stderr, "Error reading image file (comments 3)");
				free(buffer);
				return(-1);
			}
		...
		if(fgets(buffer, 2, fd) == NULL)
		{
			fprintf(stderr, "Error reading image file (comments 4)");
			free(buffer);
			return(-1);
		}
    }
    ...
    if(fgets(&buffer[1], 100, fd) == NULL)
    {
    	fprintf(stderr, "Error reading image file (size)");
		free(buffer);
		return(-1);
    }
	/* Sauvegarde des longueurs (rs,cs,depth) de l'image */
	...
	{
		fprintf(stderr, "Error reading image file (max value)");
		free(buffer);
		return(-1);
	}
	...
	//and its size
	if(rowsize(*image_decoupee)<(int64_t)width || colsize(*image_decoupee)<(int64_t)height || depth(*image_decoupee)<(int64_t)dpth)
	{
	...
	if(type_image == 5 || type_image == 7)
	{
		...
		for (profondeur=0;profondeur<profondeur_max;profondeur++)
		{
			...
			{
				if (fgets(buffer,largeur_max+1,fd) == NULL)
				{
					fprintf(stderr, "Error reading image file (contents)");
					free(buffer);
					return(-1);
				}
