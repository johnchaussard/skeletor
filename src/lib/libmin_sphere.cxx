...
//Only use this with compute_min_disk function
int32_t compute_min_disk_with_border_constraint(double *tab_point, uint32_t num_point, double *tab_point_border, uint32_t num_point_border, double *c_x, double *c_y, double *c_r)
{
	...
	else
	{
		...
		if( ((tab_point[2*num_point -2]-(*c_x))*(tab_point[2*num_point -2]-(*c_x)) + (tab_point[2*num_point -1]-(*c_y))*(tab_point[2*num_point -1]-(*c_y))) > ((*c_r)*(*c_r) + EPSILON) )
		{
...
int32_t compute_min_sphere_with_border_constraint(double *tab_point, uint32_t num_point, double *tab_point_border, uint32_t num_point_border, double *c_x, double *c_y, double *c_z, double *c_r)
{
	...
	else
	{
		...
		if( ((tab_point[3*num_point -3]-(*c_x))*(tab_point[3*num_point -3]-(*c_x)) + (tab_point[3*num_point -2]-(*c_y))*(tab_point[3*num_point -2]-(*c_y)) + (tab_point[3*num_point -1]-(*c_z))*(tab_point[3*num_point -1]-(*c_z))) > ((*c_r)*(*c_r) + EPSILON) )
		{
			//The chosen point is not in the disk...
			for(i=0; i<num_point_border; i++)
			{
...
int32_t build_sphere(double *tab_coord, uint32_t num_points, double* c_x, double* c_y, double* c_z, double* c_r)
{
	...
	else if(num_points==3)
	{
		...
		if(fabs(a)<EPSILON && fabs(b)<EPSILON && fabs(c)<EPSILON)
		{
			...
			if(fabs(tab_coord[0]-tab_coord[3])<EPSILON && fabs(tab_coord[1]-tab_coord[4])<EPSILON && fabs(tab_coord[2]-tab_coord[5])<EPSILON)
				return build_sphere(&tab_coord[3], 2, c_x, c_y, c_z, c_r);

			else if(fabs(tab_coord[0]-tab_coord[6])<EPSILON && fabs(tab_coord[1]-tab_coord[7])<EPSILON && fabs(tab_coord[2]-tab_coord[8])<EPSILON)
				return build_sphere(tab_coord, 2, c_x, c_y, c_z, c_r);

			else if(fabs(tab_coord[3]-tab_coord[6])<EPSILON && fabs(tab_coord[4]-tab_coord[7])<EPSILON && fabs(tab_coord[5]-tab_coord[8])<EPSILON)
				return build_sphere(tab_coord, 2, c_x, c_y, c_z, c_r);
	...
	else if(num_points==4)
	{
		...
		if(fabs(det_M)<EPSILON)
		{
			...
			if(fabs(r_c*r_c - ((x_c-tab_coord[9])*(x_c-tab_coord[9]) + (y_c-tab_coord[10])*(y_c-tab_coord[10]) + (z_c-tab_coord[11])*(z_c-tab_coord[11]))) > EPSILON)
			{
